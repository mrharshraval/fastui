...
export const HALFTONE_PASS_SHADER = {
  ...
  void main() {
    ...
    // Reduce scale slightly to bring lines closer together
    float halftoneSize = max(tile * max(footprintScale, 0.001), 1.0) * 0.85;
    ...
    vec3 finalColor = mix(bgColor, dashColor, alpha);
