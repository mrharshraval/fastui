...
export type CreateImageSessionOptions = {
...
export function createImageSession({
  ...
  const halftoneMaterial = new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      bgColor: { value: new THREE.Color(settings.halftone.bgColor ?? 0xffffff) },
      time: { value: 0 },
  ...
  const syncFootprint = () => {
    ...
    halftoneMaterial.uniforms.footprintScale.value = getImageFootprintScale({
      imageFit: 'contain',
      imageWidth: image instanceof HTMLVideoElement ? image.videoWidth : image.naturalWidth,
      imageHeight: image instanceof HTMLVideoElement ? image.videoHeight : image.naturalHeight,
  ...
  // Always register pointer listeners — reduced motion disables continuous
  // autonomous animation but interactive hover feedback must remain usable.
  ...
  const renderFrame = ({ deltaSeconds, elapsedSeconds }: VisualFrame) => {
    // Under reduced motion: freeze autonomous animation (waves, time-driven
    // effects) but keep interactive hover — just snap it instantly.
    halftoneMaterial.uniforms.time.value = reducedMotion ? 0 : elapsedSeconds;
    halftoneMaterial.uniforms.waveAmount.value =
      reducedMotion ? 0 : (settings.wave.enabled ? settings.wave.amount : 0);

    // Reduced motion → instant snap (easing = 1); normal → exponential ease.
    const hoverEasing = reducedMotion
      ? 1
      : 1 -
        Math.exp(
          -deltaSeconds *
    ...
    // Under reduced motion: snap pointer position instantly too.
    const followFactor = reducedMotion ? 1 : settings.pointer.follow;
    pointer.smoothedX += (pointer.mouseX - pointer.smoothedX) * followFactor;
    pointer.smoothedY += (pointer.mouseY - pointer.smoothedY) * followFactor;
    pointer.velocityX = reducedMotion ? 0 : pointer.velocityX * settings.pointer.velocityDamping;
    pointer.velocityY = reducedMotion ? 0 : pointer.velocityY * settings.pointer.velocityDamping;
