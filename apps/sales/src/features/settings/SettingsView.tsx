...
export function SettingsView({ initialUser = null }: SettingsViewProps) {
  ...
  const [activeModal, setActiveModal] = React.useState<
    "profile" | "password" | "notifications" | "logout" | "delete" | null
  >(null);
  ...
  const userNameDisplay =
    displayName || (userEmailDisplay ? userEmailDisplay.split("@")[0] : "Account");
            ...
            <div className="w-full sm:max-w-md bg-background border-none rounded-t-3xl sm:rounded-2xl p-5 shadow-none animate-in slide-in-from-bottom-6 duration-200 pb-8 sm:pb-5">
              ...
              <div className="pt-4 flex flex-col gap-4">
                    ...
                    <div className="flex items-center justify-between p-3.5 rounded-2xl bg-card border-none shadow-none">
                      <div>
                        ...
                        <p className="text-[11px] text-muted-foreground mt-0.5">
                          {pushSubscribed
                            ? "Lock-screen alerts for due reminders."
                            : "Enable to get reminder notifications."}
                        </p>
                      </div>
                      <MinimalSwitch
                        checked={pushSubscribed}
                ...
                {activeModal === "delete" && (
                  <div className="flex flex-col gap-3 pt-1">
                    <p className="text-xs text-muted-foreground">
                      This action cannot be undone. All your business records, tasks, and data
        ...
        <Card className="bg-card rounded-xl border-none shadow-none">
          <CardHeader className="pb-4">
            ...
            <CardDescription className="text-xs">
              Lock-screen alerts and sound for sales reminders
            </CardDescription>
          </CardHeader>
          <CardContent className="flex flex-col gap-4">
  ...
  if (!supported) {
    return (
      <div className="text-xs text-muted-foreground">
        Push notifications are not supported on this browser. On iPhone, add FastUI to your Home
        Screen to enable lock-screen alerts.
      </div>
    );
  }
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground">
              {isSubscribed
                ? "Reminders will trigger lock-screen alerts and sounds."
                : "Enable to get reminder alerts on your phone."}
            </p>
          </div>
        </div>
