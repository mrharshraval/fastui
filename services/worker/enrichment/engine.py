"""
FastUI Website Enrichment Engine
================================
...
logger = logging.getLogger("fastui.enrichment.engine")


class WebsiteEnrichmentEngine:
    """
    Lightweight, multi-page website enrichment engine for FastUI.
    Operates independently from queue mechanics.
    """

    def __init__(
        self,
        timeout_sec: float = 8.0,
        client: Optional[EnrichmentHttpClient] = None,
    ) -> None:
        self.client = client or EnrichmentHttpClient(timeout_sec=timeout_sec)
    ...
    def _discover_subpages(
        ...
        for a in soup.find_all("a", href=True):
            ...
            if "team" not in candidates and any(
                k in path for k in ("doctor", "dentist", "our-team", "team", "faculty", "specialist")
            ):
