"""
FastUI Playwright Scraper Engine
================================
...
RSS memory instrumentation, structured popup handling, and container memory safety.
"""
...
logger = logging.getLogger("fastui.sources.playwright")
...
class PlaywrightScraper(DiscoverySourceAdapter, ABC):
    ...
    async def _cleanup_resources(self) -> None:
        ...
        """
        # 0. Cancel any active popup auto-close tasks
        if self._popup_tasks:
            for t in list(self._popup_tasks):
                t.cancel()
    ...
    async def discover(self, params: DiscoverySearchParams) -> List[DiscoveredLead]:
        ...
        try:
            ...
            # Structured popup listener with error handling and task tracking
            async def _safe_close_popup(popup: Page) -> None:
                try:
                    if not popup.is_closed():
                        await popup.close()
                except Exception as err:
                    logger.debug(f"Notice closing popup page: {err}")

            def _handle_popup(popup: Page) -> None:
                task = asyncio.create_task(_safe_close_popup(popup), name="PlaywrightAutoClosePopup")
