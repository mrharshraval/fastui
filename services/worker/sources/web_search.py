...
class WebSearchScraper(PlaywrightScraper):
    ...
    async def extract_leads(
            ...
            for res in results[:target_limit]:
                ...
                try:
                    ...
                    snippet_el = await res.query_selector(
                        ".result__snippet, .snippet, .b_caption p, p"
                    )
                    ...
                    has_whatsapp = (
                        is_mobile
                        or "whatsapp" in snippet_text.lower()
                        or "wa.me" in snippet_text.lower()
                    )
                    whatsapp_val = (
        ...
        logger.info(
            f"WebSearchScraper extracted {len(leads)} leads (exhausted={self.is_exhausted})"
        )
        return leads
