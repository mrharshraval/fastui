...
def test_dental_specialties_analyzer():
    ...
    assert (
        "all-on-4" in treatments["Dental Implants"].keywords_matched
        or "dental implant" in treatments["Dental Implants"].keywords_matched
    )
...
async def test_orchestrator_enrich_website_flow():
    ...
    from contracts.enrichment import EnrichmentParams
    ...
    orchestrator.client.fetch_html = AsyncMock(
        return_value=(html, "https://smiledentalstudio.example.com", None)
    )
    ...
    assert (
        result.profile.brand.logo_url
        == "https://smiledentalstudio.example.com/assets/brand-logo.png"
    )
...
async def test_ssrf_redirect_blocked():
    ...
    # Mock response redirecting to loopback / metadata
    ...
    with patch.object(client, "_get_client", new_callable=AsyncMock) as mock_get_c:
        mock_c = AsyncMock()
        mock_c.get.return_value = mock_redirect_resp
        mock_get_c.return_value = mock_c

        html, url, err = await client.fetch_html("https://example.com/test")
        assert html is None
        assert "SSRF blocked" in (err or "")
