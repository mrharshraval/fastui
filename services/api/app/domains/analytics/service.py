"""FastUI Analytics Domain Service.

Aggregates real-time sales pipeline KPIs, stage distributions, and live activity streams.
"""

from sqlalchemy import desc, func, select
from sqlalchemy.ext.asyncio import AsyncSession

from app.domains.analytics.schemas import ActivityFeedItem, DashboardStatsResponse
from app.domains.businesses.models import Business
from app.domains.engagement.models import Activity
...
class AnalyticsService:
    ...
    async def get_dashboard_metrics(session: AsyncSession) -> DashboardStatsResponse:
        ...
        result = await session.execute(select(Lead.stage, func.count(Lead.id)).group_by(Lead.stage))
        stage_counts = {
            (k.value if hasattr(k, "value") else str(k)).lower(): v for k, v in result.all()
        }
