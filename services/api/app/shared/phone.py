...
and standardizes phone numbers with country calling codes across global locations.
"""
...
def resolve_country_code(location: str | None) -> str:
    ...
    # Match token from right-to-left
...
def normalize_global_phone(
    ...
    # 2. Check if raw phone explicitly had a '+' sign
...
def _format_e164_display(digits: str, target_cc: str) -> str:
    if digits.startswith("91") and len(digits) == 12:  # India (+91)
        sub = digits[2:]
        if sub[0] in "6789":  # Mobile
            return f"+91 {sub[:5]} {sub[5:]}"
        elif sub.startswith(("11", "22", "33", "44", "20", "79", "80")):  # 2-digit STD
...
def is_mobile_phone(raw_phone: str | None, location: str | None = None) -> bool:
...
def get_whatsapp_url(raw_phone: str | None, location: str | None = None) -> str | None:
