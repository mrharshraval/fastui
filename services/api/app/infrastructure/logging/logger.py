"""FastUI Enterprise Structured Logging System.
...
- Automatic ContextVars correlation ID propagation across async call stacks.
"""
...
class DevelopmentConsoleFormatter(logging.Formatter):
    """Clean, human-readable terminal formatter for local development."""
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        formatted = (
            f"{timestamp} [{level_str}] [{self.service_name}] [{record.name}]{corr_tag} "
            f"{record.getMessage()}"
        )
...
def setup_logging(service_name: str = "fastui-api", log_level: str | None = None) -> None:
