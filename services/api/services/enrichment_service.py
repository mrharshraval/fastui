"""
FastUI Business Website Enrichment Service
==========================================
Orchestrates asynchronous website enrichment via the Cloud Run Worker.
Processes extracted brand logos, practitioner details, dental clinical specialties,
conversion touchpoints, and technical quality audit scores.
Updates business master entities, provenance records, and ProspectDemo overrides.
"""
...
class EnrichmentService:
    ...
    @staticmethod
    async def enrich_business_background(business_id: int, correlation_id: Optional[str] = None) -> None:
        """
        Background worker task: calls the Worker enrichment engine, parses
        the enriched profile, and updates DB entities.
        """
        if correlation_id:
        ...
        async with AsyncSessionLocal() as session:
            ...
            if not business:
                logger.warning(f"Enrichment aborted: business_id={business_id} not found.")
                return

            if not business.website:
                logger.info(f"Enrichment skipped: business_id={business_id} ('{business.business_name}') has no website.")
                return
            ...
            try:
                # 1. Call Worker enrichment engine
                enrich_resp: EnrichmentResponse = await WorkerClient.enrich_business(
                    website=target_url,
                ...
                if not enrich_resp or not enrich_resp.success or not enrich_resp.profile:
                    logger.warning(
                        f"Enrichment returned unsuccessful for business_id={business_id}: "
                        f"status='{enrich_resp.status if enrich_resp else 'error'}', "
                        f"error='{enrich_resp.error if enrich_resp else 'No response'}'"
                    )
                    failure_activity = Activity(
                        ...
                        notes=f"Failed to enrich {target_url}: {enrich_resp.error or 'No profile returned'}",
                ...
                profile: EnrichedBusinessProfile = enrich_resp.profile
                profile_dict = profile.model_dump()

                # 2. Update or create website BusinessSource provenance
                norm_domain = LeadDeduplicator.normalize_website(target_url)
                if norm_domain:
                    loc_sig = EnrichmentService.compute_location_signature(business.address, business.city, business.postal_code)
                ...
                if website_source:
                    ...
                    website_source.last_seen_at = datetime.now(timezone.utc)
                ...
                # Also attach enrichment payload to existing Google Maps source for consolidated lookup
                for src in business.sources:
                ...
                if not business.email and profile.contact.emails:
                    ...
                    logger.info(f"Enriched email for business_id={business_id}: {business.email}")
                ...
                for demo in business.demos:
                    ...
                    logger.info(f"Updated demo overrides for token={demo.token} (business_id={business_id})")
                ...
                await session.commit()
                logger.info(f"Background enrichment completed successfully for business_id={business_id}")

            except Exception as e:
                logger.error(f"Background enrichment error for business_id={business_id}: {e}", exc_info=True)
                await session.rollback()

    @staticmethod
    def extract_enrichment_from_business(business: Business) -> Optional[EnrichedBusinessProfile]:
        """
        Helper to extract the parsed EnrichedBusinessProfile from any attached BusinessSource.
        Safely inspects loaded sources without triggering un-awaited async lazy loads.
        """
    ...
    def compute_location_signature(
        ...
        """
        Computes a deterministic location signature for multi-branch disambiguation.
        Combines normalized city, postal code, and primary street tokens.
        """
        ...
        if address:
            ...
            skip_words = {c_slug, p_code, "road", "street", "lane", "avenue", "near", "opp", "opposite", "beside", "floor", "shop", "no", "plot"}
    ...
    async def find_matching_cached_intelligence(
        ...
        prospect_postal = prospect.postal_code or LeadDeduplicator.extract_postal_code(prospect.address)
        prospect_loc_sig = EnrichmentService.compute_location_signature(prospect.address, prospect.city, prospect.postal_code)

        best_candidate: Optional[CrawledWebsite] = None
        best_score = 0.0
        ...
        for candidate in candidates:
            ...
            if prospect_phone:
                if prospect_phone in cached_phones:
                    ...
                    for b in cached_branches:
                        if b.get("phone") and LeadDeduplicator.normalize_phone(b["phone"]) == prospect_phone:
                            phone_matched = True
                            score += 0.95
                            break

            # 2. Exact location signature match
            if candidate.location_signature and candidate.location_signature == prospect_loc_sig and prospect_loc_sig != "default":
                score += 0.90
            ...
            # Conflict check: differing postal codes without phone match -> REJECT
            if prospect_postal and cached_postal and prospect_postal != cached_postal and not phone_matched:
                branch_has_postal = any(
                    b.get("address") and LeadDeduplicator.extract_postal_code(b["address"]) == prospect_postal
                    for b in cached_branches
                )
                if not branch_has_postal:
                    continue  # Conflicting postal code / separate branch, skip candidate

            # Conflict check: differing city without phone match -> REJECT
            if prospect.city and cached_addr:
                ...
                if prospect_city_clean not in cached_addr.lower() and not phone_matched:
                    ...
                    if not branch_has_city:
                        continue  # Conflicting city / separate branch, skip candidate

            # Address match with main address or branches
            ...
            if addr_match is True:
                ...
                for b in cached_branches:
                    ...
                    if b_addr:
                        ...
                        if LeadDeduplicator.are_addresses_matching(
                            prospect.address,
                            b_addr,
                            postal1=prospect_postal,
                            postal2=b_postal,
    ...
    async def get_or_enrich_website(
    ...
    ) -> Tuple[Optional[EnrichedBusinessProfile], bool]:
        """
        ...
        - Crawls and enriches fresh data if no corroboration or un-cached.
        Returns: (EnrichedBusinessProfile or None, is_reused: bool)
        """
        ...
        if not force_refresh:
            ...
            if cached_record and cached_record.extracted_data:
                try:
                    ...
                    # Evidence-based business name reconciliation on cache hit
                    brand_name = getattr(profile.brand, "brand_name", None) or getattr(profile.brand, "name", None)
                    meta_title = getattr(profile.brand, "meta_title", None)
                    resolved_name = BusinessNameNormalizer.resolve_canonical_name(
                        source_name=getattr(prospect, "source_name", None) or getattr(prospect, "raw_business_name", None) or prospect.business_name,
                    ...
                    if not prospect.has_whatsapp and (profile.contact.whatsapp_url or profile.contact.whatsapp):
        ...
        # 2. Fresh crawl and enrichment
        target_url = prospect.website.strip()
        if not target_url.startswith("http://") and not target_url.startswith("https://"):
            target_url = f"https://{target_url}"

        logger.info(f"Initiating fresh website crawl for '{target_url}' (prospect_id={prospect.id})")
        try:
            enrich_resp: EnrichmentResponse = await WorkerClient.enrich_business(
                website=target_url,
                business_name=prospect.business_name,
            )
            if not enrich_resp.success or not enrich_resp.profile:
                logger.warning(f"Enrichment returned unsuccessful for {target_url}: {enrich_resp.error}")
                return (None, False)

            profile = enrich_resp.profile
            profile_dict = profile.model_dump()

            # Store in CrawledWebsite
            ...
            # Update BusinessSource provenance asynchronously without triggering implicit lazy loading
            ...
            # Evidence-based business name reconciliation on fresh crawl
            brand_name = getattr(profile.brand, "brand_name", None) or getattr(profile.brand, "name", None)
            meta_title = getattr(profile.brand, "meta_title", None)
            resolved_name = BusinessNameNormalizer.resolve_canonical_name(
                source_name=getattr(prospect, "source_name", None) or getattr(prospect, "raw_business_name", None) or prospect.business_name,
    ...
    def personalize_demo_overrides(
        prospect: Business,
        profile: Optional[EnrichedBusinessProfile],
        ...
        Strict constraint: Uses authentic extracted data only, falling back cleanly to existing defaults.
        """
        overrides = dict(existing_overrides or {})
        if not profile:
            return overrides

        # 0. Canonical business name
        canonical = getattr(prospect, "canonical_name", None) or getattr(prospect, "business_name", None)
        ...
        if tagline and not overrides.get("tagline"):
            ...
            if not overrides.get("hero_headline") or overrides.get("hero_headline") == "Where healthy smiles begin.":
        ...
        if about and not overrides.get("about_text"):
            ...
            if not overrides.get("hero_subheadline") or overrides.get("hero_subheadline") == "Comprehensive, gentle dental care tailored to your family's needs.":
        ...
        # 8. Clinical facilities & amenities
        ...
        if profile.insurance_info and profile.insurance_info.get("providers") and not overrides.get("insurance_providers"):
            overrides["insurance_providers"] = profile.insurance_info["providers"]

        # 11. Emergency care info
        ...
        # 16. Social media accounts (Instagram, Facebook, YouTube, Twitter/X, LinkedIn)
        ...
        # 17. WhatsApp direct link
        wa_url = (
            profile.contact.whatsapp_url
            or profile.contact.whatsapp
            if profile.contact
            else None
        )
