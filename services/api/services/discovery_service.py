...
over authenticated HTTPS, then persists the results via deduplication and normalization.

Separation of concerns:
  API (this file)  — job lifecycle, deduplication, normalization, DB persistence
  Worker (Cloud Run) — Playwright scraping, in-memory aggregation
"""
...
class DiscoveryService:
    """
    Manages discovery job lifecycle and background extraction processing.
    """
    ...
    async def process_job(job_id: int) -> None:
        """
        Dispatches scraping to the Cloud Run Worker, then normalizes, deduplicates,
        and persists the returned leads into the database.
        """
        async with AsyncSessionLocal() as session:
            ...
            try:
                ...
                # Build search parameters from the stored job query
                ...
                logger.info(
                    f"Job {job_id}: Dispatching to Worker — "
                    f"'{search_params.target_audience}' in '{search_params.location}'"
                )

                # ── Call Cloud Run Worker ──────────────────────────────────────
                raw_leads: List[DiscoveredLead] = await WorkerClient.discover(search_params)
                # ─────────────────────────────────────────────────────────────

                # Check for mid-request cancellation
                await session.refresh(job)
                if job.status == JobStatus.CANCELLED:
                    logger.info(f"Job {job_id} was cancelled during worker call. Discarding results.")
                    return

                job.total_discovered = len(raw_leads)

                # Resolve location components for persistence
                ...
                # Normalize, deduplicate, and persist each lead
                for i, lead in enumerate(raw_leads):
                    ...
                    if duplicate:
                        ...
                        web_status = (
                            WebsiteStatus.WEBSITE_FOUND if lead.website else WebsiteStatus.NO_WEBSITE
                        )
