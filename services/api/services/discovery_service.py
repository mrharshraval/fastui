...
from models.schema import (
    DiscoveryJob, JobStatus, Business, Lead, Activity,
    ActivityType, WebsiteStatus, PipelineStage, LeadPriority, LeadSignal
)
...
def _run_scraper_in_worker_thread(params: DiscoverySearchParams) -> List[Any]:
    """Runs multi-source discovery in a dedicated worker thread with a guaranteed Windows Proactor loop."""
    import sys
...
class DiscoveryService:
    ...
    async def cancel_job(session, job_id: int) -> DiscoveryJob:
        ...
        if job.status in (JobStatus.COMPLETED, JobStatus.FAILED, JobStatus.CANCELLED):
            raise ConflictException(f"Job {job_id} cannot be cancelled because it is already {job.status.value}")
            
        job.status = JobStatus.CANCELLED
        job.completed_at = datetime.now(timezone.utc)
        await session.commit()
        await session.refresh(job)
        logger.info(f"Job {job_id} was successfully marked as CANCELLED.")
        return job

    @staticmethod
    async def process_job(job_id: int):
        """
        Background worker task orchestrating scraping, normalization, deduplication, and DB persistence.
        """
        async with SCRAPER_SEMAPHORE:
            async with AsyncSessionLocal() as session:
                ...
                try:
                    ...
                    target_audience = query_dict.get("target_audience") or query_dict.get("business_type") or "Businesses"
                    ...
                    logger.info(f"Job {job_id}: Commencing scraper for '{search_params.target_audience}' in '{search_params.location}'")
                    raw_leads = await asyncio.to_thread(_run_scraper_in_worker_thread, search_params)
                    
                    # Refresh job to check for cancellation during scraping phase
                    await session.refresh(job)
                    ...
                    # Parse structured location fields
                    loc_city = None
                    loc_state = None
                    loc_country = None
                    ...
                    for i, lead in enumerate(raw_leads):
                        ...
                        if duplicate:
                            ...
                            discovery_activity = Activity(
                                ...
                                channel="scraper",
                                outcome="Discovered via Google Maps",
