...
2. Businesses with the same or highly similar names are treated as separate entities
   unless there is additional evidence that they represent the same physical business.
3. Supporting identifiers (address, phone number, website, Google/place ID, coordinates)
   are required to verify that two records are duplicates.
4. If the name is identical but the location or other identifiers indicate different
   businesses (e.g. conflicting addresses, postal codes, phones, or coordinates),
...
class LeadDeduplicator:
    ...
    def are_addresses_matching(
        ...
        Returns:
          ...
          False -> Conflicting evidence (different postal codes, different building/sector numbers).
        ...
        # Different postal codes in the same region indicate different physical zones
        ...
        # House, shop, plot, or sector numbers must not conflict
    ...
    async def find_duplicate(
        ...
        Duplicate detection is NEVER based on business name alone.
        """
        # 1. Check BusinessSource external_id (Google Place ID - strongest physical identity)
        ...
        # 2. Check normalized_phone (Direct verified contact line)
        if normalized_phone:
            phone_query = select(Business).where(Business.normalized_phone == normalized_phone).limit(5)
            phone_res = await session.execute(phone_query)
            phone_candidates = phone_res.scalars().all()
            for cand in phone_candidates:
                # Check for explicit location contradiction (different postal codes or conflicting street addresses)
                addr_match = cls.are_addresses_matching(address, cand.address, postal_code, cand.postal_code, city=city or cand.city)
                if addr_match is False:
                    continue
                return cand

        # 3. Check normalized_website (Custom non-generic domain)
        if normalized_website and normalized_website not in GENERIC_DOMAINS:
            web_query = select(Business).where(Business.normalized_website == normalized_website).limit(5)
            web_res = await session.execute(web_query)
            web_candidates = web_res.scalars().all()
            for cand in web_candidates:
                # Disallow match if both have conflicting non-empty phone numbers
                if normalized_phone and cand.normalized_phone and normalized_phone != cand.normalized_phone:
                    continue
                # Disallow match if addresses/postal codes conflict
                addr_match = cls.are_addresses_matching(address, cand.address, postal_code, cand.postal_code, city=city or cand.city)
                if addr_match is False:
                    continue
                return cand

        # 4. Check business_name + Supporting Location/Business Metadata
        ...
        for cand in candidates:
            # Check negative evidence / conflicts first:
            # - Conflicting phone numbers
            if normalized_phone and cand.normalized_phone and normalized_phone != cand.normalized_phone:
                continue

            # - Conflicting custom websites
            if (
                normalized_website
            ...
            addr_match = cls.are_addresses_matching(address, cand.address, postal_code, cand.postal_code, city=clean_city or cand.city)
            if addr_match is False:
                continue

            # - Conflicting coordinates (if available)
            coord_match = None
            if latitude is not None and longitude is not None:
                # Look up coordinates on candidate's sources if available
                pass

            # Check positive supporting evidence:
            ...
            # Without supporting location/contact metadata, name alone is NEVER enough!
            # Continue checking other candidates or treat as separate entity.
